...
class Model:
    ...
    def dilate(self, imgpath, size, rawImage=None):
        ...
        if rawImage is None:
            res = self.filter(kernel, imgpath, kernelFunc=_dilate)
        else:
            res = self.convolution(kernel, rawImage, kernelFunc=_dilate)
        return res

    def erode(self, imgpath, size, rawImage=None):
        kernel = np.zeros((size, size))
        ...
        if rawImage is None:
            res = self.filter(kernel, imgpath, kernelFunc=_erode)
        else:
            res = self.convolution(kernel, rawImage, kernelFunc=_erode)
        return res
    ...
    def filter(self, kernel, imgpath, kernelFunc=None):
        img = np.array(cv2.imread(imgpath), dtype=np.uint8)
        img = img[:, :, 0]  # select R channel as gray scale
