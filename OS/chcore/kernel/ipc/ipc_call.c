...
static u64 thread_migrate_to_server(struct ipc_connection *conn, u64 arg)
{
	 ...
	 * to the server?
	 * */
	// conn->callback is not initialized here, so use server_ipc_config instead
...
u64 sys_ipc_call(u32 conn_cap, ipc_msg_t *ipc_msg)
{
	 ...
	 */
	// change the pmo cap to server's process
	// should be invoked before change of virtual address to avoid pgfault
	 ...
	 * */
	// change the virtual address to server's vmspace
...
u64 sys_ipc_reg_call(u32 conn_cap, u64 arg0)
{
	...
	// pass arg0 as parameter directly
