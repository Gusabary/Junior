...
int rr_sched_enqueue(struct thread *thread)
{
	if (!thread || !thread->thread_ctx || thread->thread_ctx->state == TS_READY
		|| !IS_VALID_AFF(thread->thread_ctx->affinity)) {
		return -1;
	}
	...
	u32 cpuid = (affinity == NO_AFF) ? smp_get_cpu_id() : affinity;
...
{
	if (!thread || !thread->thread_ctx || thread->thread_ctx->type == TYPE_IDLE) {
		return -1;
	}
	...
	struct thread *node;
	bool find = false;
	for_each_in_list(node, struct thread, ready_queue_node, &(rr_ready_queue[cpuid])) {
		if (node == thread) {
			find = true;
		}
	}
...
struct thread *rr_sched_choose_thread(void)
{
	...
	struct thread *chosen_thread = list_entry(rr_ready_queue[cpuid].next,
											  struct thread, ready_queue_node);
	rr_sched_dequeue(chosen_thread);
...
int rr_sched(void)
{
	...
	if (current_thread && current_thread->thread_ctx->type != TYPE_IDLE) {
		...
		// shouldn't reset budget here because idle thread will never be enqueued
		// current_thread->thread_ctx->sc->budget = DEFAULT_BUDGET;
	}

	struct thread *next_thread = rr_sched_choose_thread();
	// can reset budget here because idle thread can also be chosen
	next_thread->thread_ctx->sc->budget = DEFAULT_BUDGET;
