...
u64 sys_handle_brk(u64 addr) {
	 ...
	 */
	if (addr == 0) {
		retval = vmspace->user_current_heap;  // save the original heap addr
		pmo = obj_alloc(TYPE_PMO, sizeof(*pmo));
		if (!pmo) {
			ret = -ENOMEM;
			goto out_fail;
		}
		pmo_init(pmo, PMO_ANONYM, 0, 0);  // the 4th arg for PMO_ANONYM doesn't matter
	...
	vmr = vmspace->heap_vmr;  // get the allocated heap
	...
	// shrinking heap is not supported
	retval = -EINVAL;
	goto out;
