...
static int tfs_mknod(struct inode *dir, const char *name, size_t len, int mkdir)
{
	...
	// create and add dentry
	dent = new_dent(inode, name, len);
	htable_add(&dir->dentries.buckets, (u32)dent->name.hash, &dent->node);
...
int tfs_namex(struct inode **dirat, const char **name, int mkdir_p)
{
	...
	i = 0;
	memset(buff, 0, MAX_FILENAME_LEN + 1);  // important to init buff as zero
	while (1) {
		char c = *((*name) + i);
		if (!c) {
			return 0;
...
// You can use memory functions defined in libc
ssize_t tfs_file_read(struct inode *inode, off_t offset, char *buff, size_t size)
{
	BUG_ON(inode->type != FS_REG);
	BUG_ON(offset > inode->size);
  ...
  // TODO(Lab5): write your code here
	while (size > 0) {
		...
		if (page_no == inode->size / PAGE_SIZE) {
			to_read = MIN(to_read, (inode->size) % PAGE_SIZE);
		}
...
int tfs_load_image(const char *start)
{
	...
	for(f = g_files.head.next; f; f = f->next) {
		...
		dirat = tmpfs_root;  // XXX: hacking
		leaf = f->name;
		write_count = f->header.c_filesize;
		tfs_namex(&dirat, &leaf, 1);
		len = strlen(leaf);
		if (write_count == 0) {
			// filesize of dir is 0
			// XXX: '.' dir may need special handle
			tfs_mkdir(dirat, leaf, len);
		}
		else {
			// filesize of reg file is > 0
			tfs_creat(dirat, leaf, len);
			dent = tfs_lookup(dirat, leaf, len);
			tfs_file_write(dent->inode, 0, f->data, write_count);
		}
	}
			 ...
			 unsigned char type, ino_t ino)
{
	...
	unsigned short len = strlen(name) + 1 +
						 sizeof(dirp->d_ino) +     // 8
						 sizeof(dirp->d_off) +     // 8
						 sizeof(dirp->d_reclen) +  // 2
						 sizeof(dirp->d_type);     // 1
