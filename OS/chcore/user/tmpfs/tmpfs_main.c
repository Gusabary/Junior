...
static int map_buff(ipc_msg_t *ipc_msg, void *buff) {
	int cap = ipc_get_msg_cap(ipc_msg, 0);
	usys_map_pmo(SELF_CAP, cap, (u64)buff, VM_READ | VM_WRITE);
	return cap;
}
	...
	if (ipc_msg->data_len >= 4) {
      ...
      // TODO(Lab5): your code here
		...
		case FS_REQ_UNLINK:
			...
			break;
		case FS_REQ_SCAN:
			// XXX: the second argument may not be fr->offset
			cap = map_buff(ipc_msg, fr->buff);
