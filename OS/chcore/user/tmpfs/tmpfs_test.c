...
static int test_mkdir() {
  int err = fs_server_mkdir("/dir", 0666);
  return err;
}

static int test_creat() {
  int err = fs_server_creat("/file", 0666);
  return err;
}
...
static int test_deepfile()
{
  int err;
#if 0
	err = fs_server_mkdir("/dir", 0666);
  if (err != 0) return err;
#endif
	
	err = fs_server_mkdir("/dir/dir2", 0666);
  if (err != 0) return err;

	err = fs_server_creat("/dir/dir2/file", 0666);
  if (err != 0) return err;

	err = fs_server_rmdir("/dir/dir2");
	if (err != -ENOTEMPTY) return -1;
