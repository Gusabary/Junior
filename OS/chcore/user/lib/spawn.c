				  ...
				  s32 aff)
{
...

	{
		...
		pmo_requests[0].type = PMO_DATA;  // immediate allocation
...

	{
		 ...
		 */
		if (nr_pmo_map_reqs > 0) {
			usys_map_pmos(new_process_cap, pmo_map_reqs, nr_pmo_map_reqs);
		}
	}

	{
		 ...
		 * that address.
		 */
		// but what the difference between main thread and child thread
...

	{
		 ...
		 */
		stack_va = stack_top - PAGE_SIZE;  // if not -PAGE_SIZE, a pgfault at 0x8010000 will rise
