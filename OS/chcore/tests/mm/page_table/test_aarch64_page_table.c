...
void free_page(void *page)  // isn't it free_pages here?
{
...
MU_TEST(test_map_unmap_huge_page)
{
	/* Test both map_apge and unmap_page. */
	int err;
	paddr_t pa;
	...
	flags = DEFAULT_FLAGS | VMR_BLK_2M;
	err = map_range_in_pgtbl(root, va, 0x1000000, ARM64_MMU_L2_BLOCK_SIZE, flags);
	mu_assert_int_eq(0, err);

	err = query_in_pgtbl(root, va, &pa, &entry);
	mu_assert_int_eq(0, err);
	mu_check(pa == 0x1000000);

	err = unmap_range_in_pgtbl(root, va, ARM64_MMU_L2_BLOCK_SIZE);
	mu_assert_int_eq(0, err);
	...
	flags = DEFAULT_FLAGS | VMR_BLK_1G;
	err = map_range_in_pgtbl(root, va, 0x40000000, ARM64_MMU_L1_BLOCK_SIZE, flags);
	mu_assert_int_eq(0, err);

	err = query_in_pgtbl(root, va, &pa, &entry);
	mu_assert_int_eq(0, err);
	mu_check(pa == 0x40000000);

	err = unmap_range_in_pgtbl(root, va, ARM64_MMU_L1_BLOCK_SIZE);
	mu_assert_int_eq(0, err);
...
{
	// order matters, I don't know why
	// MU_RUN_TEST(test_map_unmap_huge_page);
