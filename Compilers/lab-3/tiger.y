...
exp:  lvalue  {$$ = new A::VarExp(errormsg.tokPos, $1);}
  ...
  |  ID LPAREN actuals RPAREN  /* function call */           {$$ = new A::CallExp(errormsg.tokPos, $1, $3);}
  |  ID LBRACK exp RBRACK OF exp  /* type-id[exp] of exp */  {$$ = new A::ArrayExp(errormsg.tokPos, $1, $3, $6);}
  |  ID LBRACE rec RBRACE  /* type-id{id=exp} */             {$$ = new A::RecordExp(errormsg.tokPos, $1, $3);}

  |  lvalue ASSIGN exp  /* assign */    {$$ = new A::AssignExp(errormsg.tokPos, $1, $3);}
  |  sequencing  /* (exp; exp; exp) */  {$$ = new A::SeqExp(errormsg.tokPos, $1);}
  |  LPAREN exp RPAREN                  {$$ = $2;}
...
expseq:               {$$ = nullptr;}
  |  exp              {$$ = new A::SeqExp(errormsg.tokPos, new A::ExpList($1, nullptr));}
...
one:  ID DOT ID  /* a.key */           {$$ = new A::FieldVar(errormsg.tokPos, new A::SimpleVar(errormsg.tokPos, $1), $3);}
  |  ID LBRACK exp RBRACK  /* a[1] */  {$$ = new A::SubscriptVar(errormsg.tokPos, new A::SimpleVar(errormsg.tokPos, $1), $3);}
  ;

oneormore:  one                                  {$$ = $1;}
  |  oneormore DOT ID  /* a.key.name */          {$$ = new A::FieldVar(errormsg.tokPos, $1, $3);}
  |  oneormore LBRACK exp RBRACK  /* a[1][2] */  {$$ = new A::SubscriptVar(errormsg.tokPos, $1, $3);}
  ;
...
tyfields_nonempty:  ID COLON ID           {$$ = new A::FieldList(new A::Field(errormsg.tokPos, $1, $3), nullptr);}
