...
void do_proc(FILE* out, F::ProcFrag* procFrag) {
  ...
  FILE *debug_tree_before_canon;
  debug_tree_before_canon = fopen("debug_tree_before_canon", "at+");
  procFrag->body->Print(debug_tree_before_canon, 0);
  fprintf(debug_tree_before_canon, "\n\n");
  fclose(debug_tree_before_canon);
  ...
  FILE *debug_tree;
  debug_tree = fopen("debug_tree", "at+");
  stmList->Print(debug_tree);
  fprintf(debug_tree, "\n\n");
  fclose(debug_tree);
  ...
  fprintf(out, ".set %sframesize, %d\n", TEMP::LabelString(procFrag->frame->label).c_str(), procFrag->frame->localCount * 8);
  fprintf(out, ".globl %s\n", procName.c_str());
...
int main(int argc, char** argv) {
  ...
  FILE *debug_absyn = fopen("debug_absyn", "w");
  absyn_root->Print(debug_absyn, 0);
  fprintf(debug_absyn, "\n\n");
  fclose(debug_absyn);
