    ...
    if (ch == '`') {
      ...
      switch (assem.at(i)) {
        case 's': {
          ...
          result += *s;
          // std::string s = "t" + std::to_string(nth_temp(src, n)->Int());
          // printf("---t%d\n", nth_temp(src, n)->Int());
          // result += s;
        } break;
        case 'd': {
          ...
          result += *s;
          // std::string s = "t" + std::to_string(nth_temp(dst, n)->Int());
          // result += s;
        } break;
        case 'j': {
          i++;
...
void OperInstr::Print(FILE* out, TEMP::Map* m) const {
    // errormsg.Error(0, "OperInstr Print");
  std::string result =
...
void MoveInstr::Print(FILE* out, TEMP::Map* m) const {
  ...
  // suppress move instr between the same reg
  if (m->Look(this->dst->head) == m->Look(this->src->head) && !this->dst->tail && !this->src->tail)
    return;
