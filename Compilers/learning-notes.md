...
从出口处开始分析，在最后一个节点被定值（def）的变量是在指向最后一个节点的所有边上都活跃的变量，然后按边进行活跃分析，注意有可能需要重复操作直到收敛。

+ 动态活跃：所有的分支都没有对某个变量的 def
+ 静态活跃：存在一条分支，其中没有对某个变量的 def
+ 活跃分析一般使用的是静态的活跃信息，即只要存在一条分支上有 def，就认为该变量是活跃的，这样不仅是因为更加保守，也是因为一般计算不出动态活跃信息。

### 冲突

如果 a 和 b 的活跃范围重叠，则他们是冲突的；如果用一条不能对寄存器 r 进行寻址的指令来生成 a 时，则 a 和 r 也是冲突的。

冲突可以用无向图来表示，变量是图中的点，如果两个变量冲突，则他们之间有一条边。
...
+ 构造（Build）：构造冲突图，并将和传送有关的节点之间连上虚线
+ 简化（Simplify）：一个一个地从图中删除低度数且与传送无关的节点
+ 合并（Coalesce）：用启发式算法进行保守的合并
+ 冻结（Freeze）：当简化和合并都不能再进行时，寻找一个低度数的传送有关节点，放弃对这些传送指令合并的希望，即删除与该节点相连的虚线，这将导致该节点成为传送无关的，从而使得有更多的节点可以被简化。
+ 潜在溢出（Potential Spill）：选择一个可能溢出的高度数节点，将他压入栈
+ 选择（Select）：弹出整个栈并指派颜色
+ 重写（Rewrite）：对于某一实际溢出节点的每次使用，生成一个新的 temp，在使用之前先 load；对于定值，生成一个新的 temp，在定值之后 store。这样做的好处在于 use 和 def 时刻可以是两个不同的 temp，将原本跨度很长的活跃范围变成两个在头和尾的很短的活跃范围，大大提高简化的可能性。
...
+ 标记阶段会标记所有可到达的节点
+ 清扫阶段会遍历整个堆，将标记过的节点解除标记，将未标记的节点视为垃圾回收掉

优化：

+ 递归优化为栈
+ 指针逆转：节点压栈后用来存储栈的其他元素
+ 空闲表数组：将空闲表从一条链按大小分成多条链，这样再分配的时候可以直接从链表头取出空间
...
可到达的节点和垃圾分散地排布在 from-space 中，复制式收集将垃圾剔除，并将可到达的节点紧密地排列在 to-space 中。

具体实现：Cheney 算法 + 转递（Cheney 算法本质是宽度优先地调用转递算法）

但是宽度优先带来较差的局部性，所以可以采用部分宽度优先和部分深度优先的混合算法

### 分代收集

基本思想是年轻的很有可能很快就死掉，而老的很有可能会继续活下去，所以每次收集都尽量只收集 G0 中的垃圾，为了知道 G0 中哪些节点是被指向的，需要维护一个记忆表，表中存储那些较老的代中指向 G0 中节点的节点。

### 增量式收集

收集器和编译好的程序（变异器）可以并发地执行
...
##### Last-modified date: 2019.12.10, 3 p.m.